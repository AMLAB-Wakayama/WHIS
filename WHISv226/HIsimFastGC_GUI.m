...
%    ParamHIGUI.Table_DegreeCompressionPreSet(NumComp);  %Tableの値で変換。see HIsimFastGC_InitParamHI.m
% 以下と等価。
...
% 外部関数、HIsimFastGC_GetSrcSndNrmlz2CalibTone.mに。
%
...
HISparam.HLdB_LossLinear   = ParamHIGUI.HLdB_LossLinear;  %  HIsimFastGCで使用。
%HISparam.HLdB_LossCompression  % 13 Dec18 これは設定不要。 HIsimFastGCで使わない。
...
HISparam.HearingLevelVal         = ParamHIGUI.HearingLevelVal;    % 13 Dec 18 HIsimFastGCで使用　Gain補正のため
...
%%%%%%%%%%%%%%%%%%追加　9Jul15
    ...
    function myBMCallback(src, eventdata)
        ...
        if (7.5 > Cp(1,1))&&(Cp(1,1) > 0.5)
            ...
            if ParamHIGUI.GUI.KeyPress == 1;%マゼンタのオージオグラムを操作するとき
