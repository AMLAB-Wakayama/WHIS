...
%    Modified:   9 May  21 % ParamHIbatch.AudiogramNumの説明追加
%    Modified:  10 May 21 % 説明追加:  WHIS(HIsim)の計算では、CalibToneレベルにRMS値でNormalizeされた音声を使う
%    Modified:  20 Oct  21 % Batchの場合、HISparam.AllowDownSampling = 0;  
%
%   function [HIsimSnd, SrcSnd] = HIsimBatch(SndIn, ParamHIbatch);
%   INPUT:  SndIn : input sound 
%           ParamHIbatch : parameters
%                        AudiogramNum : audiogram select (1~8の数値)  %  See also HIsimFastGC_InitParamHI.m
%                                             1.example 1
%                                             2.立木2002 80yr
%                                             3.ISO7029 70yr 男
%                                             4.ISO7029 70yr 女
...
% 音圧レベル合わせ
%%%%%%%%%%%%%%%%%%%%
disp(['CalibTone_SPLdB  = '  num2str(ParamHIbatch.SPLdB_CalibTone) ' (dB)']);
disp(['SrcSnd_SPLdB      = '  num2str(ParamHIbatch.SrcSndSPLdB) ' (dB)']);
...
ParamHIbatch.SndLoad = SndIn;
 [ParamHIbatch] = HIsimFastGC_GetSrcSndNrmlz2CalibTone(ParamHIbatch); % RMSでNormalize
 disp(['SrcSnd_StrNormalizeWeight = ' ParamHIbatch.SrcSnd_StrNormalizeWeight]);
 SrcSnd = ParamHIbatch.SrcSnd; % calbration tone のRMS値でnormalizeされたSrcSnd
...
if  nn ~= 3
    error(['Something wrong with HL_LossComp_LossLin  --> Check NumComp' int2str(NumComp)]);
end
...
HISparam.SrcSnd_StrNormalizeWeight  = ParamHIbatch.SrcSnd_StrNormalizeWeight;  %  == 'RMS';  % 10 May 21
...
% for introducing FreeField/DefuseFiled/ELC   5 Nov 21
HISparam.OutMidCrct = ParamHIbatch.OutMidCrct;

% Batchの場合、downsamplingしない。2021/10/20
% HISparam.AllowDownSampling = 1;    % 0.4RT ---   cf. 0.7RTwhen HISparam.AllowDownSampling = 0.
HISparam.AllowDownSampling = 0;    % 0.7RTwhen HISparam.AllowDownSampling = 0.

% HIsimFastGCに入力
%  WHIS(HIsim)の計算では、CalibToneレベルにRMS値でNormalizeされた音声を使う。--- LAeqではないことに注意　 10 May 21
%  これはGCFBのMeddisHairCellレベル(RMS Normalize)での要請から。
[HIsimSnd, ~] = HIsimFastGC(ParamHIbatch.SrcSnd, HISparam); 
