...
if isfield(HISparam,'OutMidCrct') == 0  
    HISparam.OutMidCrct = 'ELC';   % default
end
GCparam.OutMidCrct = HISparam.OutMidCrct;

GCparam.Ctrl = 'level'; % level estimation only

tic
disp('##### GCFB for level estimation (Excitation Pattern) #####');
% [cGCout, pGCout, GCparam, GCresp] = GCFBv211(SrcSnd,GCparam); % original 
[cGCout, pGCout, GCparam, GCresp] = GCFBv211_ff(SrcSnd,GCparam); % FreeField from GCFBv231
tElps(1) = toc;
